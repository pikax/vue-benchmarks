...
- **One table per surface and work class, one cell per metric.** RSS and Alloc are rendered `min / max / avg` in a single cell. Status is a marker on the tool name — **❌** error · **⏭** skipped — and per-row detail is in the collapsible **Notes** under each table, where a note shared by every row is written once. `n/a` means the platform cannot measure that number; `–` means the row never ran.
...
One table per surface and, where work differs, per comparison class. Each metric is one `min / max / avg` cell; status is a marker on the name (❌ error · ⏭ skipped) and per-row detail is under **Notes** below each table. `n/a` = not measurable on this platform; `–` = the row never ran.
...
| ------------------------------------------------------------------------ | --------------------: | --------------------: | ------: | ----: | ------: | ------: |
| Vize compileSfc loop (full SFC, 1T) vapor-prod                           | 15.16 / 15.16 / 15.16 |    0.94 / 0.94 / 0.94 |   30.72 | 111.7 |   27.33 |       3 |
| fervid compileSync (1T) vdom-prod                                        | 15.93 / 15.93 / 15.93 |    0.77 / 0.77 / 0.77 |   30.15 | 111.8 |   28.32 |       3 |
| Vize compileSfc loop (full SFC, 1T) vdom-prod                            | 16.65 / 16.65 / 16.65 |    0.87 / 0.87 / 0.87 |   27.86 | 111.6 |   25.56 |       3 |
| Vize compileSfcBatchWithResults (full SFC, Rayon global pool) vapor-prod | 17.43 / 17.43 / 17.43 |    0.84 / 0.84 / 0.84 |     n/a |   n/a |   13.61 |       3 |
| Vize compileSfcBatchWithResults (full SFC, Rayon global pool) vdom-prod  | 18.11 / 18.11 / 18.11 |    0.77 / 0.77 / 0.77 |     n/a |   n/a |   12.27 |       3 |
| Verter compileMany (fresh host, stateless runtime-render) vapor-prod     | 37.71 / 37.71 / 37.71 |    0.83 / 0.83 / 0.83 |   88.95 | 142.1 |   61.15 |       3 |
| Verter compileMany (fresh host, stateless runtime-render) vdom-prod      | 37.84 / 37.84 / 37.84 |    0.82 / 0.82 / 0.82 |   86.56 | 142.2 |   59.29 |       3 |
