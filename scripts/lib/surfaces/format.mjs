...
export async function runFormatSurface(fixtureDir, options) {
  ...
  if (oxfmt) {
    variants.push({
      ...
      invocation: "cli",
      notes:
        "oxfmt --write (fresh copy each run) · pinned 0.65.0 routes a full .vue file through its bundled Prettier formatFile callback in worker threads; the native binding orchestrates the call, but Vue parsing/printing is the bundled Prettier path. Re-audit this package path after upgrades.",
      measure: () => {
