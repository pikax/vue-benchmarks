...
```bash
pnpm confirm:typecheck          # per-plant + all-plants (local)
pnpm confirm --all              # CI path: typecheck is one spawn per tool
```

`docs/typecheck.md` is generated by confirm. Do not hand-edit the tables. CI (`test.yml` and the Benchmark confirm job) uses `--all`. A Benchmark dispatch on `main` commits the file plus a README summary.
