...
| `.github/workflows/test.yml`      | Harness + confirm on every PR and `main` push. On `main`, commits `docs/typecheck.md` (Linux times)           |
...
`docs/typecheck.md` is generated by that command. Do not hand-edit the tables. On `main`, Linux CI re-runs confirm and commits the file so published times/RSS are Linux, not a laptop run.
